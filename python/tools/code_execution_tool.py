...
# Get logger for tool subsystem
_tool_log = get_logger(LogSubsystem.TOOL)


def powershell_quote(code: str) -> str:
    """
    Quote a string for use in PowerShell command line.
    PowerShell uses double quotes and escapes internal double quotes with backtick.
    """
    # Escape backticks and double quotes for PowerShell
    escaped = code.replace('`', '``').replace('"', '`"').replace('$', '`$')
    return f'"{escaped}"'


def quote_for_shell(code: str, use_powershell: bool = False) -> str:
    """
    Quote code appropriately for the current shell environment.
    """
    if use_powershell:
        return powershell_quote(code)
    else:
...
class CodeExecution(Tool):
    ...
    async def execute(self, **kwargs) -> Response:
        ...
        # Register process in registry for tracking
        code_preview = str(self.args.get("code", ""))[:50]
        registry = get_registry()
        entry = ProcessEntry(
            command=f"{runtime}:{code_preview}...",
            metadata={"runtime": runtime, "session": session}
        )
        registry.register(entry)
        _tool_log.debug(f"Code execution started", extra={"entry_id": entry.id, "runtime": runtime})
        ...
        try:
            ...
            _tool_log.debug(f"Code execution completed", extra={"entry_id": entry.id, "duration_ms": entry.duration_ms})

        except Exception as e:
            # Mark as failed
            registry.mark_failed(entry.id, exit_code=1, error=str(e))
            _tool_log.error(f"Code execution failed", extra={"entry_id": entry.id, "error": str(e)})
            raise
    ...
    async def execute_python_code(self, session: int, code: str, reset: bool = False):
        # Use PowerShell-compatible quoting on Windows with local shell
        use_ps = runtime.is_windows() and not self.agent.config.code_exec_ssh_enabled
        escaped_code = quote_for_shell(code, use_powershell=use_ps)
    ...
    async def execute_nodejs_code(self, session: int, code: str, reset: bool = False):
        ...
        use_ps = runtime.is_windows() and not self.agent.config.code_exec_ssh_enabled
        escaped_code = quote_for_shell(code, use_powershell=use_ps)
        # On Windows, use node directly; on Linux use the helper script
        if use_ps:
