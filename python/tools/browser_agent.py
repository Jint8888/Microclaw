...
class State:
    ...
    async def _initialize(self):
        ...
        # NOTE: New browser-use API (CDP-based) uses _cdp_add_init_script instead of browser_context.add_init_script
        if self.browser_session:
            try:
    ...
    def kill_task(self):
        ...
        if self.browser_session:
            try:
                ...
                loop.run_until_complete(self.browser_session.stop()) if self.browser_session else None
                loop.close()
            except Exception as e:
    ...
    async def get_selector_map(self):
        ...
        if self.use_agent and self.use_agent.browser_session:
            try:
                # New browser-use API: use get_browser_state_summary instead of get_state_summary
                await self.use_agent.browser_session.get_browser_state_summary(
