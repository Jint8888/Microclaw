...
# ============================================================
# Enhanced Memory System Auto-Replacement
# When MEMORY_ENHANCED_ENABLED=true, Memory.get() returns EnhancedMemory
# ============================================================

def _apply_enhanced_memory_patch():
    """Apply monkey patch to replace Memory with EnhancedMemory."""
    import os
    if os.getenv("MEMORY_ENHANCED_ENABLED", "false").lower() in ("true", "1", "yes"):
        try:
            from python.helpers.memory_enhanced import EnhancedMemory
            
            # Store original methods
            Memory._original_get = Memory.get
            Memory._original_get_by_subdir = Memory.get_by_subdir
            
            # Replace with enhanced versions
            @staticmethod
            async def enhanced_get(agent: Agent):
                """Enhanced Memory.get that returns EnhancedMemory."""
                return await EnhancedMemory.get(
                    agent,
                    enable_hybrid=True,
                    enable_cache=os.getenv("MEMORY_CACHE_ENABLED", "true").lower() in ("true", "1", "yes"),
                    cache_size=int(os.getenv("MEMORY_CACHE_SIZE", "1000")),
                )
            
            @staticmethod
            async def enhanced_get_by_subdir(
                memory_subdir: str,
                log_item: LogItem | None = None,
                preload_knowledge: bool = True,
            ):
                """Enhanced Memory.get_by_subdir that returns EnhancedMemory."""
                # For dashboard and other subdir-based access, use original for now
                # as EnhancedMemory.get requires an agent
                return await Memory._original_get_by_subdir(
                    memory_subdir, log_item, preload_knowledge
                )
            
            Memory.get = enhanced_get
            Memory.get_by_subdir = enhanced_get_by_subdir
            
            PrintStyle.standard("✓ Enhanced Memory System enabled (FTS5 + Hybrid Search)")
        except Exception as e:
            PrintStyle.error(f"Failed to enable Enhanced Memory: {e}")
            PrintStyle.standard("Falling back to standard Memory system")


# Apply the patch when module is loaded
_apply_enhanced_memory_patch()
