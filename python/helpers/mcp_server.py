...
import fastmcp.settings as fastmcp_global_settings
...
class DynamicMcpProxy:
    ...
    def reconfigure(self, token: str):
        ...
        # Update global fastmcp settings (new API - use direct attributes, not .settings)
        fastmcp_global_settings.message_path = message_path
        fastmcp_global_settings.sse_path = sse_path
        ...
        with self._lock:
            self.sse_app = create_sse_app(
                ...
                sse_path=sse_path,
                auth=mcp_server.auth,  # New API: use auth instead of auth_server_provider + auth_settings
                debug=fastmcp_global_settings.debug,
            ...
            self.http_app = self._create_custom_http_app(
                http_path,
                mcp_server.auth,  # New API: use auth property
                fastmcp_global_settings.debug,
    ...
    def _create_custom_http_app(self, streamable_http_path, auth, debug, routes):
        ...
        # Add StreamableHTTP routes (simplified for new fastmcp API)
