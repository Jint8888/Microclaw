...
use "pip" "npm" in "terminal" to install packages; on Windows use "winget" or "choco"
...
on Windows environment, use PowerShell commands (Get-ChildItem, Set-Location, Invoke-WebRequest, etc.)
usage:
...
2 execute terminal command (PowerShell on Windows)
~~~json
{
    "thoughts": [
        "Need to do...",
    ...
    "headline": "Listing files in current directory via PowerShell",
    "tool_name": "code_execution_tool",
    "tool_args": {
        "runtime": "terminal",
        "session": 0,
        "code": "Get-ChildItem -Path .",
    }
}
~~~
